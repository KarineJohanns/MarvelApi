function convertMarvelApitoMarvel(marvelDetail) {
  ...
  character.name = marvelDetail.names;
  ...
  character.comics = marvelDetail.comics.items;
...
marvelApi.getCharacterDetails = (characterDetails) => {
  const offset = 0;
  const ts = "1675253895";
  ...
  return fetch(characterDetails.url)
    .then((response) => response.json())
    .then((jsonBody) => jsonBody.data.results)
    .then(convertMarvelApitoMarvel);
};
